...
import com.dds.theecogame.data.remote.statistics.dto.toCurrentUserRanking
...
class StatisticsRepositoryImpl : StatisticsRepository {
    ...
    override suspend fun getUserRanking(userId: Int): Flow<Resource<CurrentUserRanking>> = flow {
        emit(Resource.Loading())

        val response = api.getRankingFromUser(userId)

        if (response.isSuccessful)
            emit(Resource.Success(response.body()!!.toCurrentUserRanking()))
        else
            emit(Resource.Error(response.message()))
    }
